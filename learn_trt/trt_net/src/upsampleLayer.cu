...
namespace nvinfer1 {
  ...
  void UpsampleLayerPlugin::forwardGpu(const Dtype* input,Dtype * output,
    ...
    int numElem = N * C * H * W;
    // grid shape (numElem + mThreadCount - 1) / mThreadCount)
    // block shape (mThreadCount)
    // upsample 采用的复制的方式完成上采样
  ...
  int UpsampleLayerPlugin::enqueue(
    ...
    // 如果输入输出的shape一致，不需要进行计算直接进行数据拷贝
